...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['update'])) {
        ...
        $sqlUpdate = "UPDATE package SET type=?, location=?, details=?, price=? WHERE id=?";
        $stmt = $conn->prepare($sqlUpdate);
        $stmt->bind_param("ssssi", $type, $location, $details, $price, $id);
        $stmt->execute();
        $stmt->close();
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<button onclick=\"showUpdateForm({$row['id']}, '{$row['type']}', '{$row['location']}', '{$row['details']}', {$row['price']})\"><i class='fas fa-edit'></i></button>";
                ...
                echo "<tr><td colspan='6' style='text-align: center; font-weight: bold;'>No packages available.</td></tr>";
            }
            ?>
        </tbody>
    </table>
...
<script>
    function showUpdateForm(id, type, location, details, price) {
