...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    for ($i = 1; $i <= $numPlaces; $i++) {
        $places[] = $conn->real_escape_string($_POST['place_' . $i]);
    }
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath)) {
        $stmt = $conn->prepare("INSERT INTO package (type, location, price, details, places, days, image_path) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssissss", $type, $location, $price, $details, $placesSerialized, $days, $targetFilePath);
   ...
   <br><br>
      ...
      <form method="POST" enctype="multipart/form-data" id="packageForm">
         ...
         <div class="form-group">
            ...
            <input type="number" name="num_places" id="numPlaces" min="1" max="10" required>
         </div>
